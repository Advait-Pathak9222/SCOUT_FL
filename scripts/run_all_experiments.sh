...
#
#   bash scripts/run_all_experiments.sh [DEVICE] [--quick] [--from STEP] [key=value ...]
#
...
OVERRIDES=("fl.device=$DEVICE" "${EXTRA_OVERRIDES[@]}")
...
if [ "$FROM" -le 1 ]; then
    ...
    python -m scout_fl.experiments.run_fl_synthetic \
        --config scout_fl/configs/ablation.yaml $QUICK --override "${OVERRIDES[@]}"
    ...
    python -m scout_fl.experiments.run_fl_synthetic \
        --config scout_fl/configs/ablation_vismaya.yaml $QUICK --override "${OVERRIDES[@]}"
...
if [ "$FROM" -le 2 ]; then
    ...
    python -m scout_fl.experiments.run_fl_synthetic \
        --config scout_fl/configs/campaign_main.yaml $QUICK --override "${OVERRIDES[@]}"
...
if [ "$FROM" -le 3 ]; then
    ...
    python -m scout_fl.experiments.run_campaign $QUICK --override "${OVERRIDES[@]}"
...
if [ "$FROM" -le 4 ]; then
    ...
    python -m scout_fl.experiments.run_regret --config scout_fl/configs/campaign_main.yaml --rounds 300 --override "${OVERRIDES[@]}"
fi
