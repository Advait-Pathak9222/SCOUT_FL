...
\caption{System model of \scout{}. In each round $t$ the base station broadcasts the global model $\bw_t$ and the active set to the clients. Each client $k$ in the active set performs local stochastic gradient descent (SGD) to form an update $\Delta\bw_{k,t}$, applies channel-inversion precoding $b_{k,t}=\rho_t/h_{k,t}$, and transmits over the shared uplink. The superposition at the AirComp multiple-access channel realises the model aggregate $\bw_{t+1}=\bw_t+\bm{y}_t$ directly, corrupted only by receiver noise $\bm{n}_t$. The same transmissions illuminate the targets, and the bistatic echoes drive a multistatic radar estimator that assembles the per-target Fisher information $\bJ_m(\calS)$ from the anisotropic per-client contributions~\eqref{eq:fim}. The \scout{} primal--dual selector then greedily maximises the composite learning-plus-sensing utility and updates the dual price $\mu$ on the realised aggregation MSE, producing the next active set $\calS_{t+1}$.}
\label{fig:system}
\end{figure*}
...
The operating point is deliberately constrained, and the absolute accuracy figures should be interpreted accordingly. At $T=150$ training has converged; over the final $25$ rounds the accuracy curve varies by less than $0.1$ percentage points, so the results do not reflect a truncated training horizon. The accuracy ceiling is set by the federated regime, comprising a compact CNN, a $20{,}000$-image subsample distributed over $100$ clients, $10\%$ participation per round, non-IID data, and AirComp aggregation MSE incurred every round, rather than by model capacity. Controlled runs verified that enlarging the network does not raise the plateau in this regime and destabilises training under the aggregation noise. Since this is a client-selection study, the quantity of interest is the relative gap between selectors at a fixed, converged operating point. Both final and peak accuracy are reported for every method.
...
Fig.~\ref{fig:gapdist} widens the view from the single operating point of Fig.~\ref{fig:frontier} to the full campaign and the full sensing-aware field. For each of the $25$ operating points, every method's final accuracy is measured against the best sensing-aware method at that point, and the resulting gap distribution is shown together with each method's Pareto-optimal share. \scout{} combines the smallest accuracy gaps of any method that retains a high Pareto share (median $1.3$ percentage points, never exceeding $4.7$) with non-dominance at $96\%$ of the points. The only method with smaller gaps, OTA-FL-ISCC, attains them by abandoning the sensing objective, which Fig.~\ref{fig:frontier} shows places it outside the sensing-competitive band, while the methods with comparable sensing investment (Asaad, CRB-Only, Sensing-Only) sit $8$--$13$ percentage points behind the field.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_gap_dist.pdf}
\caption{Distribution of the accuracy gap to the best sensing-aware method at each operating point ($25$ OFAT points, $5$ seeds each; gap computed on seed-averaged final accuracy, so the best method at a point has zero gap there). Boxes span the interquartile range with whiskers at $1.5\times$IQR; dots are the individual operating points. Methods are ordered by median gap; for the five methods whose median exceeds the $12$~pp axis cap, only the below-cap points are drawn and the median is printed. The percentage above each method is its Pareto-optimal share of the $25$ points under the final-round CRB convention; under the round-mean convention the shares agree within $\pm16$~pp across the field and are identical ($96\%$, i.e.\ $24/25$ points) for \scout{}. Methods at $0\%$ carry no label.}
\label{fig:gapdist}
\end{figure}
...
Table~\ref{tab:cross} repeats the comparison on five datasets, reporting final and peak accuracy. Among the sensing-competitive methods, \scout{} attains the highest accuracy on every dataset by both measures, and its CRB remains within a competitive band of the sensing-heavy baselines throughout. Crucially, the geometric advantage provided by the sensing utility is mathematically decoupled from the dimensionality of the learning task. As established in (4) and (5), the Fisher information matrix depends exclusively on the physical coordinates of the active set and the waveform parameters, remaining strictly independent of the $d$-dimensional gradient embeddings evaluated in (9). The absolute accuracy naturally scales with dataset complexity, while the relative performance margin of SCOUT-FL is preserved across both lower-dimensional (e.g., Fashion-MNIST) and higher-dimensional (e.g., CIFAR-100) feature distributions. This decoupling is empirically evidenced in Table~\ref{tab:cross}, where SCOUT-FL maintains a stable, competitive CRB floor alongside the highest relative accuracy across all evaluated datasets. The advantage is therefore not specific to a particular dataset.
...
\caption{Selection overhead per round ($N=100$, $K=10$, mean\,$\pm$\,std over rounds and seeds). The greedy selection of \scout{} scales linearly and requires approximately one-seventh of the overhead of the state-of-the-art Asaad reference.}
\label{fig:runtime}
\end{figure}

\subsection{Adaptation Under a Budget Change}\label{sec:adapt}
The cognitive loop is exercised dynamically by cutting the aggregation budget in the middle of training. At round $75$ the budget drops from $\varepsilon=10^{-3}$ to $1.5\times10^{-4}$, which is below the realised error floor of $2.0\times10^{-4}$ at this operating point, so a constraint that has been slack for the whole first half suddenly binds. Fig.~\ref{fig:adapt} shows the response over $5$ seeds. The dual price is identically zero before the cut. It rises within one round of the first violation, peaks near $0.6$, and settles around $0.4$ once the selection conforms. Every seed produces a budget-feasible active set within two rounds of the cut. After recovery the time-averaged aggregation MSE is $1.51\times10^{-4}$ against the budget of $1.50\times10^{-4}$, so the loop tracks the constraint in time average to within about one percent, while individual rounds oscillate around the boundary ($70\%$ strictly feasible per round). This is exactly the behaviour guaranteed by Proposition~\ref{prop:pd}, which bounds the time-averaged violation rather than every round. Under the same disturbance the hard gate is feasible in every round but finishes at $40.8\pm3.5\%$ accuracy, whereas the soft loop finishes at $42.3\pm3.8\%$.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig12_adaptation.pdf}
\caption{Cognitive adaptation to a mid-run budget cut ($5$ seeds, mean and $\pm$std band). At round $75$ the aggregation budget drops from $10^{-3}$ to $1.5\times10^{-4}$, below the realised error floor, so the constraint binds. \textbf{(a)} The dual price $\mu_t$ is zero throughout the slack phase, rises within one round of the first violation, and settles once the selection conforms. \textbf{(b)} The realised aggregation MSE returns to the new budget within two rounds in every seed and tracks it in time average thereafter. The hard-gate ablation under the same disturbance is feasible in every round but pays $1.5$ percentage points of final accuracy.}
\label{fig:adapt}
\end{figure}

Fig.~\ref{fig:epsgate} sweeps the static budget from the slack operating point into the binding regime. Panel (a) exposes the mechanism. The soft loop spends the whole budget, with a realised time-averaged MSE within one percent of $\varepsilon$ at every binding level, until the natural error floor of the operating point is reached. The hard gate is conservative and leaves ten to twenty percent of the budget unused, because its per-client feasibility threshold must hold in every realisation. Panel (b) shows the accuracy consequence. The unused budget costs the hard gate $4.6$ percentage points at the marginally binding level $\varepsilon=2\times10^{-4}$ ($44.4$ against $39.8$). Deep in the binding regime both variants must restrict the selection to strong channels and their accuracies coincide within seed noise. At the slack operating point the two are indistinguishable, consistent with Table~\ref{tab:ablation}. The advantage of the soft constraint therefore concentrates where the budget is tight enough to matter but loose enough that discarding whole candidate sets wastes information.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig13_epsgate.pdf}
\caption{Budget sweep from the binding regime to the slack operating point ($5$ seeds). \textbf{(a)} Realised time-averaged aggregation MSE against the prescribed budget. The soft primal--dual loop follows the identity line to within one percent until the error floor of the operating point is reached, whereas the hard gate leaves ten to twenty percent of the budget unused. \textbf{(b)} Final accuracy. The unused budget costs the hard gate $4.6$ percentage points at the marginally binding level, the two coincide within seed noise deep in the binding regime, and they are indistinguishable when the budget is slack.}
\label{fig:epsgate}
\end{figure}

\subsection{Achievability of the CRB}
Fig.~\ref{fig:rmse} relates the CRB optimised by the algorithm to the localisation error attained by a receiver. The FIM and the corresponding CRB in this work are derived strictly from the multistatic radar observation model. They are unrelated to the Fisher information that is sometimes used to characterise neural network loss landscapes, and they carry no dependence on the training dynamics. For a fixed geometry, a Monte-Carlo ML localisation is performed and its root-mean-square error (RMSE) is plotted against $\sqrt{CRB}$ across a 30 dB SNR range. The empirical RMSE tracks the CRB floor at every SNR, confirming Proposition 5. The CRB differences reported in this section therefore correspond to differences in achievable localisation accuracy.
The recent literature highlights sensing mutual information (SMI) as a robust metric for stochastic ISAC environments \cite{zhang2026integrated}. Yet the CRB is explicitly adopted in this work because its relationship with the Fisher information matrix directly captures the spatial geometry of the active set, and because the CRB remains the metric around which contemporary ISAC waveform, beamformer, and rate trade-off designs are built~\cite{wang2024cramer,zhou2026crb}. The aggregate FIM is additive across clients, ensuring that the log-determinant sensing utility is monotone submodular (Proposition 1). This structural property is what enables the near-optimal approximation guarantee of the greedy scheduler. Although SMI provides valuable insights into the fundamental limits of random communication waveforms, translating it into a submodular set-function for combinatorial client selection remains non-trivial.
...
In summary, \scout{} is the most accurate sensing-competitive scheduler in this study on both final and peak accuracy, while methods that post higher raw accuracy do so only by leaving the sensing-competitive band; across all $25$ operating points its accuracy gap to the field's best never exceeds $4.7$ percentage points (Fig.~\ref{fig:gapdist}). It is Pareto-optimal in $96\%$ of operating points under both CRB conventions (Fig.~\ref{fig:gapdist}), its accuracy lead over the strongest baselines persists across five datasets, the primal--dual constraint secures that lead without violating the aggregation-MSE budget, a mid-run budget cut is absorbed within two rounds with the time-averaged error held at the new budget (Fig.~\ref{fig:adapt}), and the selection overhead is low. \scout{} does not minimise the CRB; where the sensing requirement is extreme, dedicated sensing methods attain a lower CRB at a substantial accuracy cost.

% ============================================================= VII. CONCLUSION
\section{Conclusion}\label{sec:conclusion}
This paper studied client selection for ISAC over-the-air federated learning as a decision that jointly determines learning speed and sensing quality, and proposed \scout{}, a scheduler that scores candidate active sets by a geometry-aware, submodular sensing utility coupled to a diversity-based learning term, and that respects the AirComp aggregation-MSE budget through a self-tuning primal--dual price rather than a hard gate. The selector operates as a cognitive control loop. It observes the radio environment and the realised aggregation error, decides the active set, and adapts its dual price from the outcome of each round. Greedy selection is $(1-1/e)$-optimal for the composite utility, sharpened further under a curvature refinement, with the scope of the guarantee characterised for the regime in which the aggregation penalty binds. The dual loop is asymptotically feasible and no-regret, a property that a mid-run budget cut confirms experimentally, the optimised CRB is empirically achievable, and a convergence bound shows that the adaptive price converts the aggregation budget into an explicit guarantee on the trained model. Evaluated against sensing-aware baselines, \scout{} attains the highest accuracy of any method that remains sensing-competitive, lies on the accuracy--CRB Pareto frontier in $96\%$ of operating points (Fig.~\ref{fig:gapdist}), and outperforms the strongest baselines across five datasets at a fraction of the selection overhead of the state of the art. The underlying mechanism is interpretable, in that angular diversity across the active set, rather than raw SNR, improves the sensing estimate. Future work includes mobile targets with a tracking-aware FIM, multi-antenna clients with beam selection incorporated into the greedy step, and joint treatment of power control with the dual price.
...
\[
F(\bw_{t+1})\le F(\bw_t)+\nabla F(\bw_t)^{\!\top}(\bw_{t+1}-\bw_t)+\tfrac{L}{2}\lVert\bw_{t+1}-\bw_t\rVert^2 .
\]
...
\bibitem{zheng2024otafliscc} J.~Zheng \emph{et al.}, ``Over-the-air federated learning client selection in integrated sensing, communication and computation,'' in \emph{Proc.\ IEEE ICC Workshops}, 2024.
\bibitem{du2024fediscc} Y.~Du \emph{et al.}, ``Integrated sensing, communication and computation for over-the-air federated learning,'' \emph{IEEE Internet Things J.}, 2024.
\bibitem{wen2025isccairfeel} D.~Wen \emph{et al.}, ``Integrated sensing-communication-computation for over-the-air edge AI (ISCC-Air-FEEL),'' \emph{arXiv:2508.15185}, 2025.
...
\bibitem{asaad2025} S.~Asaad, H.~Wang, and H.~Tabassum, ``Sensing-aware device scheduling for over-the-air federated edge learning,'' \emph{IEEE Trans.\ Wireless Commun.}, 2025 (arXiv:2501.06334).
\bibitem{hu2025differentially} S.~Hu, X.~Yuan, W.~Ni, X.~Wang, and E.~Hossain, ``Differentially private wireless federated learning with integrated sensing and communication,'' \emph{IEEE Trans.\ Wireless Commun.}, 2025.
...
\bibitem{zhang2026integrated} D.~Zhang \emph{et al.}, ``Integrated sensing and communications over the years: An evolution perspective,'' \emph{IEEE Commun. Surveys Tuts.}, vol.~28, 2026.
\end{thebibliography}
