...
Fig.~\ref{fig:rmse} relates the CRB optimised by the algorithm to the localisation error attained by a receiver. The FIM here is derived strictly from the multistatic radar observation model and is unrelated to the Fisher information sometimes used to characterise neural network loss landscapes. For a fixed geometry, a Monte-Carlo ML localisation is performed and its root-mean-square error (RMSE) is plotted against $\sqrt{CRB}$ across a 30 dB SNR range. The empirical RMSE tracks the CRB floor at every SNR, confirming Proposition 5. The CRB differences reported in this section therefore correspond to differences in achievable localisation accuracy.
